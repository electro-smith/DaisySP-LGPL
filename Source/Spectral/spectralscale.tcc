...
void SpectralScale<FFT_SIZE, OVERLAP, WINDOW_SIZE>::Init(
...
template <size_t FFT_SIZE, size_t OVERLAP, size_t WINDOW_SIZE>
SpectralBuffer<FFT_SIZE>& SpectralScale<FFT_SIZE, OVERLAP, WINDOW_SIZE>::Process(SpectralBuffer<FFT_SIZE>& fsig_in)
{
...
//   }
  ...
  if (lastframe < fsig_in.framecount) {
    ...
    if (keepform) {
      ...
      int32_t j;
      for (i=j=0; i < N; i+=2, j++)
        fenv[j] = logf(ftmp[i] > 0.0 ? ftmp[i] : 1e-20);
        ...
        for(int n = 0; n < N; n++)
        {
          ...
          for (i=j=0; i < N/2; i++, j+=2) {
            if (keepform > 1) {
              if (fenv[i] < ceps[i])
                fenv[i] = ceps[i];
              if ((logf(ftmp[j]) - ceps[i]) > 0.23f) cond = 1;
            }
            else
              {
                fenv[i] = expf(ceps[i]);
    ...
    if(keepform) {
      for (i = 2, chan = 1; i < N; chan++, i += 2) {
        ...
        if (newchan < N && newchan > 0) {
          ...
          fsig_out_.frame[newchan + 1] = (float) (ftmp[i + 1] * pscal);
        }
      }
    } else {
      for (i = 2, chan = 1; i < N; chan++, i += 2) {
    ...
    for (i = 2; i < N; i += 2) {
      if (isnan(fsig_out_.frame[i])) fsig_out_.frame[i] = 0.0f;
